...
class Attack:
    ...
    def get_transformations(self, current_text, original_text=None, **kwargs):
        ...
        if  self.goal_function.use_timer:
            start_time = self.goal_function.timer.start()
        ...
        if  self.goal_function.use_timer:
            enlapsed_time = self.goal_function.timer.stop(start_time)
            self.goal_function.timer.update_transformation_time(enlapsed_time)
    ...
    def _filter_transformations_uncached(
        ...
        for C in self.constraints:
            ...
            if  self.goal_function.use_timer:
                enlapsed_time = self.goal_function.timer.stop(start_time)
                self.goal_function.timer.update_constraints_time(enlapsed_time, type(C).__name__)
    ...
    def _attack(self, initial_result):
        ...
        if self.goal_function.use_timer:
            enlapsed_time = self.goal_function.timer.stop(start_time)
            self.goal_function.timer.update_attack_time(enlapsed_time)
        return result
